...
(at your option) any later version: http://www.gnu.org/licenses/gpl-3.0.en.html.
...
write_bmp(const char *filename, int width, int height, char *rgb)
{
	...
	for (i = height - 1; i >= 0; i--)
	{
		for (j = 0; j < width; j++)
		{
			ipos = 4 * (width * i + j); // changed to 4 to convert our 32-bit representation (with 8 bits wasted) into compact 24-bit one
			line[3 * j] = rgb[ipos + 2];
