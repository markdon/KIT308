//TODO: include the header than contains InterlockedIncrement
...
//TODO: add extra parameter that specifies the location of the shared memory
void mandelbrot(unsigned int threadID, unsigned int iterations, float centrex, float centrey, float scaley, 
	unsigned int samples, unsigned int width, unsigned int height, unsigned int* out, int* nextStart)
{
	...
	//TODO: instead of a for-loop, iy should get it's value from the shared memory 
	//TODO: and a while loop should be used to stop when the shared memory value has exceeded the height desired
	//TODO: to ensure the shared memory location isn't accessed by two threads simultaneously, use InterlockedIncrement
	for (unsigned int iy = 0; iy < height; iy += 1)
	{
		for (unsigned int ix = 0; ix < width; ix += 1)
		{
			...
			//TODO: update this to calculate the exact memory location to write the value to each time (i.e. iy * width + ix)
