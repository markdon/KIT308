// Tutorial03b_DynamicLines.cpp 
// Multithreaded (CPU) Mandelbrot generation with dynamic load balancing (one thread generated per line)
	...
	unsigned int samples, unsigned int width, unsigned int height, unsigned int* out)
{
	...
	for (unsigned int i = 0; i < height; i++) {
		...
		//NOTE: this code will currently crash as it will make more threads than we've allocated space for
		//TODO: if we've already created enough threads (i.e. i >= threads), wait until a thread finishes (with WaitForMultipleObjects)
		//TODO: set index to be the index of the thread that finished
		if (i >= threads){
		...
		threadData[index].iterations = iterations;
		threadData[index].centrex = centrex;
		threadData[index].centrey = centrey - (scaley * 0.5f) + (i * (scaley / height));
		threadData[index].scaley = scaley / height;
		threadData[index].samples = samples;
		threadData[index].width = width;
		threadData[index].height = 1;
		threadData[index].out = out + i * width;

		threadHandles[index] = CreateThread(NULL, 0, mandelbrotThreadStart, (void*) &threadData[index], 0, NULL);
	}
