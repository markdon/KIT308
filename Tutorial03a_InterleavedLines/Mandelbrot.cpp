...
//TODO: add an extra parameter that specifies how many lines to skip each time (i.e. the number of threads doing the calculation)
void mandelbrot(unsigned int threads, unsigned int threadID, unsigned int iterations, float centrex, float centrey, float scaley, unsigned int samples, unsigned int width, unsigned int height, unsigned int* out)
{
	...
	//TODO: change the starting line (based on which thread it is)
	//TODO: change the amount of lines to step through the image between each iteration (based on the thread count)
	for (unsigned int iy = threadID ; iy < height; iy += threads)
	{
