...
struct ThreadData 
{
	...
	//TODO: store the total number of threads doing the calculation
...
DWORD __stdcall mandelbrotThreadStart(LPVOID threadData)
{
	...
	//TODO: pass in the extra parameter
	mandelbrot(data->threads, data->threadID, data->iterations, data->centrex, data->centrey, data->scaley, data->samples, data->width, data->height, data->out);
	...
	unsigned int samples, unsigned int width, unsigned int height, unsigned int* out)
{
	...
	for (unsigned int i = 0; i < threads; i++) {
		//TODO: adjust the parameters so that each thread gets the same area to render
		//TODO: add the number of threads (which is the same as the gap for each thread to leave between lines)
		//TODO: correct the starting location in memory to store the first line
